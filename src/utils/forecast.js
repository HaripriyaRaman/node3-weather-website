...
request({url,json: true},(error,response)=>{                            /* Usage of shorthand syntax */
    ...
    if(error){
        ...
        callback(undefined,"It is currently " + response.body.current.temperature + " degrees out. There is a " + response.body.current.precip + "% chance of rain. Weather Description:" + response.body.current.weather_descriptions[0])
